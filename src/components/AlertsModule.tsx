...
export type AlertItem = {
  ...
  source: 'TAREA' | 'TRÁNSITO' | 'PRÉSTAMO' | 'INCIDENCIA' | 'GUÍA' | 'DAÑADO'
  title: string
  detail: string
  date?: string | null
  ...
  async function reload() {
    ...
    for (const transit of transits.data ?? []) {
      ...
      if (days >= 30) {
        rows.push({
          ...
          reference: transit.guide_no || transit.reference || String(transit.id),
        })
      }
    }
    ...
    for (const loan of loans.data ?? []) {
      if (loan.return_date && new Date(loan.return_date + 'T23:59:59').getTime() < Date.now()) {
        rows.push({
          ...
          reference: loan.os_no || loan.material_no || String(loan.id),
        })
      }
    }
    ...
    for (const item of damaged.data ?? []) {
      rows.push({
        ...
        reference: item.material_no || String(item.id),
      })
    }
      ...
      {loading ? (
        ...
        <div className="alert-list">
          {visible.map((item) => (
            <button
              type="button"
              className={`alert-row alert-row-action severity-${item.severity.toLowerCase()}`}
              key={item.id}
            ...
            >
              ...
              <span className="alert-row-open"><span>Tratar</span><ChevronRight size={17}/></span>
            </button>
          ))}
        </div>
      ) : (
